...
class SupplierPaymentRepository {
  ...
  DatabaseReference _ref() {
    ...
    return _isGlobal
        ? database.ref('supplier_payments')
        : database.ref('supplier_payments/$_currentUid');
  }

  String get _collectionPath =>
      _isGlobal ? 'supplier_payments' : 'supplier_payments/$_currentUid';

  String _itemPath(String ownerUid, String id) {
    return _isGlobal
        ? 'supplier_payments/$ownerUid/$id'
        : 'supplier_payments/$_currentUid/$id';
  }
    ...
    if (_isGlobal) {
      for (final userEntry in value.entries) {
        ...
        for (final entry in userData.entries) {
          ...
          if (remote.deleted == 1) {
            if (local != null) {
              await _localDb.delete('supplier_payments', remote.id);
              changed = true;
            }
            continue;
          }
  ...
  Future<void> _pushRow(Map<String, Object?> row) async {
    ...
    if (_database == null) {
      ...
      await _database!.ref('supplier_payments/$ownerUid/$id').set(payload);
    } else {
    ...
    if (isDeleted) {
      ...
      await _localDb.markClean('supplier_payments', id);
    }
  }
  ...
  _RemoteRecord<SupplierPayment> _fromJson(
    String id,
    String ownerUid,
